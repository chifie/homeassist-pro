...
import { BriefcaseBusiness, CalendarClock, Check, Plus, Star, TrendingUp, X } from "lucide-react";
...
const kpis = [
  { icon: BriefcaseBusiness, label: "Jobs this month", value: "18", hint: "+4 vs last month" },
  { icon: TrendingUp, label: "Earnings", value: "$6,420", hint: "Paid out weekly" },
  { icon: Star, label: "Rating", value: "4.9", hint: "214 reviews" },
  { icon: CalendarClock, label: "Response time", value: "38 min", hint: "Top 10% of pros" },
];
...
const incoming = [
  { customer: "Dana Alves", job: "Consumer unit upgrade", when: "Today, 16:00", value: "$480" },
  { customer: "Ben Carter", job: "Outdoor lighting circuit", when: "Thu, 10:00", value: "$310" },
  { customer: "Maya Roth", job: "Socket not working", when: "Fri, 08:30", value: "$120" },
];

const reviews = [
  { name: "Hannah W.", rating: 5, text: "Clear pricing, tidy work, explained everything. Booking again." },
  { name: "Ben Carter", rating: 5, text: "Second job with Marcus — reliable and genuinely knowledgeable." },
];
    ...
    <PageShell>
      <div className="container-page py-10">
        <header className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-4 sm:flex sm:flex-wrap sm:justify-between">
          <div className="flex min-w-0 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="truncate text-sm text-muted-foreground">Master Electrician · Austin, TX</p>
            </div>
          </div>
        ...
        <section className="mt-8 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          {kpis.map((k) => (
            <div key={k.label} data-reveal className="lift rounded-2xl border bg-card p-5 shadow-soft">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-[1.5fr_1fr]">
          <section data-reveal className="rounded-2xl border bg-card shadow-soft">
            ...
            <ul className="divide-y">
              {incoming.map((r) => {
                ...
                return (
                  <li key={r.customer} className="flex flex-wrap items-center justify-between gap-3 p-5">
                    <div className="min-w-0">
                    ...
                    {done ? (
                      <Badge className="bg-success/15 text-success-foreground dark:text-success">Accepted</Badge>
                    ) : (
                      <div className="flex gap-2">
                        <Button size="sm" onClick={() => setHandled((h) => [...h, r.customer])}>
                          <Check size={15} aria-hidden /> Accept
                        </Button>
                        <Button size="sm" variant="outline" aria-label={`Decline ${r.job}`}>
          ...
          <section data-reveal className="rounded-2xl border bg-card shadow-soft">
            ...
            <ul className="divide-y">
              {listings.map((l) => (
                <li key={l.name} className="flex items-center justify-between gap-3 p-5">
                  <div className="min-w-0">
        ...
        <div className="mt-10 grid gap-6 lg:grid-cols-2">
          <section data-reveal className="rounded-2xl border bg-card p-6 shadow-soft">
            <h2 className="text-lg font-semibold">Profile management</h2>
            <form className="mt-5 grid gap-5 sm:grid-cols-2" onSubmit={(e) => e.preventDefault()}>
              <div className="grid gap-2">
                <Label htmlFor="pro-name">Display name</Label>
                <Input id="pro-name" defaultValue="Marcus Hale" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="pro-trade">Profession</Label>
                <Input id="pro-trade" defaultValue="Master Electrician" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="pro-rate">Hourly rate</Label>
                <Input id="pro-rate" defaultValue="$85" />
              </div>
              <div className="grid gap-2">
                <Label htmlFor="pro-area">Service area</Label>
                <Input id="pro-area" defaultValue="Austin, TX · 25 mi" />
              </div>
          ...
          <section data-reveal className="rounded-2xl border bg-card p-6 shadow-soft">
            ...
            <div className="mt-5 space-y-4">
              {reviews.map((r) => (
                <article key={r.name} className="rounded-xl bg-secondary/60 p-4">
